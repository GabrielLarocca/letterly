...
export function PricingPlans({ onSelectPlan, selectedPlan }: PricingPlansProps) {
  return (
    <div className="w-full max-w-4xl mx-auto mb-8">
      <h2 className="text-2xl font-bold text-center mb-6" id="pricing-heading">Escolha seu plano perfeito ✨</h2>
      <div className="grid md:grid-cols-3 gap-6" role="radiogroup" aria-labelledby="pricing-heading">
        {plans.map((plan) => (
          <Card
            key={plan.name}
          ...
          >
            ...
            <ul className="space-y-3 mb-6" aria-label={`Recursos do plano ${plan.name}`}>
            ...
            <Button 
              ...
              aria-label={`${plan.buttonText} - ${plan.price}`}
            >
              {plan.buttonText}
            </Button>
          </Card>
